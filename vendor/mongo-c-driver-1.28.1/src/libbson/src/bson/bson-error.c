                 ...
                 size_t buflen BSON_MAYBE_UNUSED) /* IN */
{
   ...
   // Note: glibc's strerror_r returns char*, but musl's returns int (POSIX).
   // We handle both by checking the return type at compile time.
#if defined(__GLIBC__)
   ret = strerror_r (err_code, buf, buflen);
#else
   /* musl and other libcs: strerror_r returns int (POSIX/XSI-compliant) */
